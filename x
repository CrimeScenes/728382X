...
local function getDynamicFov(targetPosition)
    ...
    -- Use the new Ranges structure to determine the dynamic FOV
    local range = getgenv().Ranges
    if distance <= range.CloseRange.MaxDistance then
        return range.CloseRange.FovRadius
    elseif distance <= range.MidRange.MaxDistance then
        return range.MidRange.FovRadius
    elseif distance <= range.FarRange.MaxDistance then
        return range.FarRange.FovRadius
    else
        return range.FarRange.FovRadius  -- Default to Far FOV if the target is beyond the defined ranges
    end
end
